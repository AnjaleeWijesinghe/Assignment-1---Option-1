...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        def _setup_locators():
            nonlocal input_locator, output_locator, action_locator
            if is_chat:
        ...
        input_locator = output_locator = action_locator = None
        try:
            _setup_locators()
        except Exception as e:
        ...
        def _reload_page():
            """Reload the page and re-find locators after a timeout."""
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            print(f"Testing [Row {row_index}]: {singlish_input[:60]}")

            # Allow up to 2 attempts per row (retry after reload)
                ...
                try:
                    ...
                    if action_locator:
                        try:
                            ...
                            # Button might be stuck as "Transliterating...", wait & retry
                    ...
                    if not actual_output and prev_output:
                        # Output didn't change - try reading once more
                        current = _read_output(is_chat, output_locator)
                        if current and current != prev_output:
                            actual_output = current

                    _set_cell_value(ws, row_index, actual_col_idx, actual_output if actual_output else "")
                    ...
                    if attempt == 0:
                        print(f"  Error (attempt 1): {str(e)[:80]}")
                        if _reload_page():
                            continue  # retry this row
                    print(f"  Error in UI interaction: {str(e)[:80]}")
                    try:
                    ...
                    if args.save_every and int(args.save_every) > 0:
                        try:
                            ...
                            pass
                    # If too many consecutive errors, reload preemptively
